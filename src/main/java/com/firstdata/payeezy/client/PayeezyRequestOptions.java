...
import static com.firstdata.payeezy.client.APIResourceConstants.SecurityConstants.*;
...
public class PayeezyRequestOptions {
    ...
    protected Map<String,String> getSecurityKeys(String payLoad) throws NoSuchAlgorithmException, UnsupportedEncodingException, InvalidKeyException{
        Map<String,String> returnMap=new HashMap<>();
            ...
            returnMap.put(APIKEY, this.getApiKey());
            returnMap.put(TIMESTAMP, Long.toString(System.currentTimeMillis()));
            returnMap.put(TOKEN, this.getToken());
            returnMap.put(APISECRET, this.getSecret());
            returnMap.put(PAYLOAD, payLoad);
            returnMap.put(AUTHORIZE, getMacValue(returnMap));

            // Removed after generating HMAC
            returnMap.remove(APISECRET);
            returnMap.remove(PAYLOAD);
        return returnMap;
    }
